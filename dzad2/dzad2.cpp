	...
	if (mode == "encryption") {
		...
		string text;
			...
			for (unsigned  int i = 0; i < text.length(); i++)
			{
				...
				chislo = chislo >> 2;
				chislo = pow(predkray, 15) + pow(kray, 14) + chislo; // крайний станет вторым предкрай ( второй с конца ) первым 
		...
		//0000 0000 0000 0000 
		int number1 = chislo / pow(2, 8);
		int number2 = chislo % pow(2,8);
